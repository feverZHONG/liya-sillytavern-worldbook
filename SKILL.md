...
- `check` 的预算不再拿整册合计吓人：有分流就报「单卡最大 N 条 / X token」（**按卡累加**——同一张卡可跨多个分流组，取组最大值会低估），整册合计单独标「不代表单卡注入」
...
9. **`characterFilter` 在判定链最前**（早于 sticky／delay／`@@activate`／constant）→ 想「常驻且限知情」写得出来：过滤先生效，名单外一律跳过。群聊按**当前发言成员**判（见 `12b` §2）。
...
- **「单卡最大」要按卡累加，不能取分流组的最大值**（2026-09-25 实踩）：同一张卡可以同时落在多个 `characterFilter` 分组里（例：既在「5 人知情组」又在「受害链组」）→ 取组最大值会**低估**（本册真值 4 条/534 token 被报成 3 条/349）。已收成 `per_card_totals()` 并进自测。tag 类无法枚举成员 → 当伪卡 `tag:xxx` 单列（宁可高估）。
