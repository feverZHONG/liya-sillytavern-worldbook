...
### 群聊里 depth_prompt（PList）会**叠加**（2026-09-26 查证，待本机实证）

群聊生成时各字段的处理**不一样**，做双卡／多卡同场前必须知道：

| 字段 | 群聊行为 |
|:--|:--|
| `system_prompt` | 用当前生成角色的，**不聚合** |
| `description` / `personality` / `scenario` / `mesExamples` | 看群的 `generation_mode`：默认各用各的；`APPEND` 才合并全成员 |
| **`depth_prompt`（PList）** | ⚠️ **全成员的一份不落，全部注入**（`getGroupDepthPrompts` 收集每个启用成员的 depth prompt） |

**后果**：同场角色的**人格标签会互相出现在对方的 prompt 里**（谁发言，头上都挂着别人的 PList）。
实测（两场样本）：一对声线**刻意做反**的角色（一个爱用语气助词软收尾、一个零助词零感叹号讲逻辑）在群聊里**零串线**——
但这是**卡的声线锚做得足够硬**换来的，不是机制没风险；声线接近的角色同场要另测。

**发言激活**（`NATURAL`）：先看有没有被点名（点名 = 必答，盖过概率）→ 再按 `talkativeness` 掷点（大于随机值才激活）→ 都没激活则随机挑一个。
实践口径：群聊发言频率靠 `talkativeness` 调（安静角色 0.2-0.3，外向 0.6-0.7），别指望靠卡面文字控住。
